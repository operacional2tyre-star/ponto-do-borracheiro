...
export default async function handler(req, res) {
  ...
  function detectCategory(name, nuvemshopCategories) {
    ...
    // PRIORIDADE 1: Verifica categorias da Nuvemshop com correspondencia exata
    if (Array.isArray(nuvemshopCategories)) {
      for (const cat of nuvemshopCategories) {
        ...
        // Correspondencia parcial - verifica palavras completas
        for (const [keyword, category] of Object.entries(CATEGORY_MAP)) {
          // Verifica se a palavra inteira existe na categoria
    ...
    // PRIORIDADE 2: Verifica o nome do produto - palavras completas
    const upper = (name || '').toUpperCase();

    // Pneus - verifica palavra completa
...
$code | Out-File -FilePath "api\products.js" -Encoding utf8
