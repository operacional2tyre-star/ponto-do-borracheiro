...
export default function Home() {
  ...
  const loadProducts = async () => {
    ...
    try {
      ...
      sessionStorage.setItem('products_cache', JSON.stringify(validProducts));
    } catch (e) {
  ...
  const handleTouchEnd = (e) => {
    ...
    if (Math.abs(diff) > 50) goToSlide(diff > 0 ? 'next' : 'prev');
  };
  ...
  return (
    <div className="px-4 py-3 space-y-5">
      ...
      <div className="relative">
          ...
          >
            {bannerOffers.map((offer, idx) => (
              <div key={idx} className="w-full flex-shrink-0">
                <button onClick={() => setSelectedCategory(offer.category)} className="block w-full">
                  <img src={offer.productImg} alt={offer.title} className="w-full h-[220px] object-cover" draggable="false" />
                </button>
              </div>
            ))}
          </div>
        ...
        <button onClick={() => goToSlide('prev')} className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-black/30 backdrop-blur-sm rounded-full flex items-center justify-center text-white active:scale-90 transition-all">
          <ChevronLeft size={18} />
        </button>
        <button onClick={() => goToSlide('next')} className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-black/30 backdrop-blur-sm rounded-full flex items-center justify-center text-white active:scale-90 transition-all">
      ...
      <div className="flex items-center gap-2.5">
        ...
        <button onClick={() => navigate('/search')} className="w-11 h-11 bg-white border border-gray-200 rounded-2xl flex items-center justify-center text-red-600 active:scale-95 transition-all shadow-xs flex-shrink-0">
      ...
      <div>
        <div className="flex items-center justify-between mb-3 px-0.5">
          <h2 className="font-extrabold text-gray-900 text-base tracking-tight">
            ...
            {filteredProducts.length > 0 && <span className="ml-2 text-sm font-normal text-gray-400">({filteredProducts.length})</span>}
          </h2>
          {(selectedCategory !== 'Todos' || searchTerm) && (
            <button onClick={() => { setSelectedCategory('Todos'); setSearchTerm(''); }} className="text-xs font-bold text-red-600 flex items-center gap-0.5">
              <X size={13} /> Limpar
            </button>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-10 bg-white rounded-3xl border border-gray-100 p-6">
            ...
            <button onClick={() => { setSelectedCategory('Todos'); setSearchTerm(''); }} className="mt-3 text-xs font-bold text-red-600 hover:underline">Limpar filtros</button>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-2 gap-3.5">
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 mt-6 pb-2">
                <button onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1} className="w-9 h-9 flex items-center justify-center rounded-xl border border-gray-200 bg-white text-gray-600 disabled:opacity-30 active:scale-95 transition-all shadow-xs">
                  <ChevronLeft size={18} />
                </button>
